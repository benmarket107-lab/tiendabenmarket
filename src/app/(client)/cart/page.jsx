...
import { Trash2, Minus, Plus, ArrowRight, Truck, Store } from 'lucide-react';
...
export default function CartPage() {
  ...
  if (cart.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl shadow-sm border border-slate-100 mt-10 max-w-5xl mx-auto">
        ...
        <Link href="/" className="btn-primary flex items-center gap-2 text-lg px-6 py-3">
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-20 md:pb-0">
        <div className="lg:col-span-2 space-y-4">
          {cart.map((item) => (
            <div key={item.id} className="card p-3 sm:p-4 flex flex-row items-center gap-3 sm:gap-6 group hover:border-primary/20 transition-colors relative">
          ...
          <div className="card p-6 bg-slate-50 border-slate-100">
            ...
            <div className="border-t border-slate-200 pt-4 mb-8">
              <div className="flex flex-col mb-2">
                ...
                <span className="text-3xl font-black text-slate-600 text-right mt-2">{formatCurrency(grandTotal)}</span>
              </div>
            ...
            <div className="mt-4 text-center">
              <Link href="/" className="text-sm font-semibold text-slate-500 hover:text-primary hover:underline">
                Continuar comprando
              </Link>
            </div>
          </div>
